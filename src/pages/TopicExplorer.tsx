...
> = {
  ...
  clean: {
    ...
    reviews: [
      ...
      { stars: 4, date: "Mar 6, 2026", text: "Modern, clean, and well-maintained interior.", sentiment: "Positive", themes: ["Cleanliness", "Ambiance"], rounded: true, helpful: 11 },
  ...
  return (
    <DashboardLayout>
                ...
                <div className="h-1.5 w-full bg-foreground/[0.04] rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: theme.sentiment > 70 ? "rgb(34, 197, 94)" : theme.sentiment > 50 ? "rgb(245, 158, 11)" : "rgb(239, 68, 68)",
                    }}
                  />
                </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        {/* COLUMNA MEDIDOR Y PALABRAS CLAVE */}
        <div className="glass-card p-6 flex flex-col justify-between gap-6">
          ...
          <SentimentGauge value={staticThemes.find((t) => t.id === selectedTheme)?.sentiment || 50} />

          <div className="space-y-4">
            <div>
              <span className="text-[10px] font-bold uppercase tracking-wider text-green-600 block mb-2">High Frequency Vectors (+)</span>
            ...
            <div>
              <span className="text-[10px] font-bold uppercase tracking-wider text-red-600 block mb-2">Critical Risk Nodes (-)</span>
              <div className="flex flex-wrap gap-1.5">
              ...
              <div key={i} className="p-4 rounded-xl border border-foreground/[0.03] bg-foreground/[0.01]/40 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-sm ${
                    rev.sentiment === "Positive" ? "bg-green-50 text-green-700 border border-green-200" : "bg-red-50 text-red-700 border border-red-200"
                  }`}>
                    {rev.sentiment}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center text-[10px] text-muted-foreground pt-1 border-t border-foreground/[0.02]">
                  <div className="flex gap-1">
                    {rev.themes.map((t) => (
                      <span key={t} className="bg-foreground/[0.04] px-1.5 py-0.5 rounded-sm">{t}</span>
                    ))}
                  </div>
                  <span>{rev.date}</span>
                </div>
